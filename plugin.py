...
def get_cyanide_images(limit=ITEMS_PER_PAGE, offset=0, randomize=False):
    ...
    path = xbmc.translatePath(addon.DATA_PATH + "/" + filename + ".txt")
    ...
    Utils.save_to_file(content=items,
                       filename=filename,
                       path=addon.DATA_PATH)
    return items
